...
class ServerCompare:
    ...
    while True:
      ...
      if (TIME == False):
        ...
        # 수신한 메시지 있음
          ...
          #printImg = ResultImg
          #tkImg = Image.fromarray(printImg)
          #pLabel = Label( Tk(), image = tkImg) # self.scr_servDisplay
          #pLabel.pack(expand=1, anchor=CENTER)
          #photo = tk.PhotoImage(file="sample1.jpg")
          #pLabel = tk.Label(self.scr_servDisplay, image = photo)
          #pLabel.pack(expand=1, anchor=CENTER)
  ...
  def serv_send(self):
    ...
    nowMinute = now.minute
    msgToCli = "출결 초기화 \n"
    self.scr_servDisplay.insert(tk.INSERT,"<< " + msgToCli + " ("+ str(now.time()) +")" )
...
def person_detection(img_name, type):
    ...
    # Yolo Load
    net = cv2.dnn.readNet("C:\Python_project\yolo\yolov3.weights", "C:\Python_project\yolo\yolov3.cfg")
    classes = []
...
def person_cognition():
    while(True):
        ...
        img_list = ["sample1.jpg", "sample1.jpg", "sample1.jpg"]
        max_person = 0
        person_num = 0
        for i in range(3):
